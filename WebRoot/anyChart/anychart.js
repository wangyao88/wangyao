...
AnyChart.register = function(stock) {
    ...
    stock.id = "__AnyChart___" + AnyChart.utils.getChartUUID();     
...
AnyChart.disposeFlashObjectInIE = function(obj) {
    ...
    if (obj.parentNode) {
    	obj.parentNode.removeChild(obj);
    	//added by tiger on 2014/7/4 17:49:52
    	obj = null;
    	//end add
    }
};
...
//added by tiger on 2014/7/8 17:52:11
AnyChart.utils.formatStr = function(str,l){
	var s = str + '';
	while(s.length<l){
		s = '0'+ str;
	}
...
AnyChart.utils.getChartUUID = function() {  
	var now = new Date();	
	return  now.getFullYear() + ''
		+ AnyChart.utils.formatStr((now.getMonth() +1),2)
		+ AnyChart.utils.formatStr(now.getDate(),2)
		+ AnyChart.utils.formatStr(now.getHours(),2)
		+ AnyChart.utils.formatStr(now.getMinutes(),2)									
		+ AnyChart.utils.formatStr(now.getSeconds(),2)
		+ AnyChart.utils.formatStr(now.getMilliseconds(),3)
}  
//end add
//--------------------------------------------------------------------------------------
//Firefox print fix
...
AnyChart.prototype = {
    ...
    dispose: function() {
    	//added by tiger on 2014/7/9 18:27:06
		//把派遣事件的函数先去除 
		if(this.flashObject){
			this.flashObject['dispatchEvent'] == null;
			//this.flashObject.object.stop();
			//this.flashObject.object.stopPlay();			
		}
		//flash对象执行销毁动作后，使用一个傀儡对象来替代它，用以接受还未执行的事件
		var div = document.createElement('div');
		div.id = this.id;
		div.style.display = 'none';
		div.SetReturnValue = function(){
			//alert('SetReturnValue invoked!');	
		}
    ...
    remove: function() {
        ...
        if (AnyChart && AnyChart.charts) {
            for (var i = AnyChart.charts.length-1;i>=0; i--) {
        ...
        if (AnyChart && AnyChart.chartsMap && this.id != null) {
            ...
            //added by tiger on 2014/7/4 15:58:56,删除该属性
            delete AnyChart.chartsMap[this.id];
        }
    },
